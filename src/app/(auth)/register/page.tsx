...
function RegisterForm() {
  ...
  const handleGoogleSignUp = async () => {
    ...
    setError("");
    try {
      await loginWithGoogle("applicant");
    ...
    <div className="flex-1 flex flex-col items-center justify-center p-8">
      <div className="w-full max-w-sm">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
          Step {step} of 2 — {step === 1 ? "Your account credentials" : "Optional KYC pre-fill"}
        </p>

        {/* Google */}
        {step === 1 && (
          <>
            <button onClick={handleGoogleSignUp} disabled={googleLoading} className="w-full flex items-center justify-center gap-3 py-2.5 px-4 rounded-xl border border-gray-200 dark:border-navy-700 bg-white dark:bg-navy-900 text-gray-700 dark:text-gray-300 text-sm font-medium hover:bg-gray-50 dark:hover:bg-navy-800 transition-all mb-5 disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        <form onSubmit={handleNext} className="space-y-4">
          {step === 1 ? (
            ...
            <>
              <Input label="Company PAN" value={form.pan} onChange={(e) => update("pan", e.target.value.toUpperCase())} placeholder="AABCP1234C" hint="Optional — can be added later in profile" />
              <Input label="GST Number" value={form.gstin} onChange={(e) => update("gstin", e.target.value.toUpperCase())} placeholder="27AABCP1234C1Z5" hint="Optional" />
              <div className="p-3 rounded-lg bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-900 text-xs text-amber-700 dark:text-amber-400">
                You'll be asked to complete your full profile and KYC details in the next step.
              </div>
            </>
          )}
        ...
        <p className="text-center text-sm text-gray-500 dark:text-gray-400 mt-6">
          ...
          <Link href="/login?portal=applicant" className="text-navy-600 dark:text-navy-300 font-medium hover:underline">
            Sign in
          </Link>
        </p>
      </div>
