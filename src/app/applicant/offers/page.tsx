...
import { Search, X, CheckCircle2, Building2, Star } from "lucide-react";
import { cn } from "@/lib/utils";
import { BankOffer, BGApplication } from "@/types";
...
export default function OffersPage() {
  ...
  const filtered = mockBGApplications.filter((bg) =>
  ...
  const handleAcceptOffer = async (offer: BankOffer) => {
    ...
    toast.success(`Offer from ${offer.bank_name} accepted! Your BG is now in the issuance pipeline.`);
    setSelectedBG(null);
    ...
    <>
      ...
      <div className="portal-content space-y-6">
        ...
        <Table>
          ...
          <TableBody>
            {filtered.length === 0 ? (
              <TableEmpty message="No BG applications found" icon={<Search size={24} />} />
            ) : (
              filtered.map((bg) => (
                <TableRow key={bg.bg_id} onClick={() => bg.offers.length > 0 && setSelectedBG(bg)}>
                  <TableCell>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setSelectedBG(null)}>
            <div className="bg-white dark:bg-navy-900 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-6 space-y-4">
                {selectedBG.offers.map((offer, i) => (
                  <div
                    key={offer.offer_id}
                    className={cn("rounded-xl border p-5 transition-all", offer.status === "ACCEPTED" ? "border-green-300 dark:border-green-800 bg-green-50 dark:bg-green-950/20" : "border-gray-200 dark:border-navy-700 bg-white dark:bg-navy-800")}
                  >
                    ...
                    {offer.status === "PENDING" && (
                      <Button
                        ...
                        loading={acceptingOffer === offer.offer_id}
